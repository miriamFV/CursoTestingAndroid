...
class GetProductsUseCaseTest {
    ...
    @Test
    fun givenPromotionEndingNow_whenInvoke_thenItShouldBeIncluded() =
        runTest {
            // Given
            ...
            // When
            val result: List<ProductWithPromotion> =
                (
                    useCase(
    ...
    @Test
    fun givenActivePromotion_whenTimeAdvances_thenPromotionShouldNotBeLongerReturned() =
        runTest {
            // Given
            val now: Instant = Instant.parse("2026-04-03T10:00:00Z")
            val clock: FakeSystemClock = FakeSystemClock().apply { setTime(now) }
    ...
    @Test
    fun givenInStockOnlyEnabled_whenProductsGoesOutOfStock_thenItShouldBeFiltered() =
        runTest {
            // Given
            val productId = "product-id"
            val product: Product =
